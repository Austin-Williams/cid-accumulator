...
export class ReadOnlyIpfsAdapter implements IpfsAdapter {
  ...
  /**
   * Get a block by CID from IPFS via the configured gateway.
   */
  async get(cid: CID<unknown, 113, 18, 1>): Promise<Uint8Array> {
    // Gateway expects: {gatewayUrl}/{CID}
  ...
  async put(_cid: CID<unknown, 113, 18, 1>, _data: Uint8Array): Promise<void> { return; }
  async pin(_cid: CID<unknown, 113, 18, 1>): Promise<void> { return; }
  async provide(_cid: CID<unknown, 113, 18, 1>): Promise<void> { return; }
}
