...
IPFS can work with any DAG structure, and thus any shape of Merkle tree. For this project, gas efficiency is critical. When appending a new leaf to a Merkle tree in the EVM setting, each intermediate node update requires an SLOAD and _at least one_SSTORE (the number of SSTOREs varies depending on the chosen Merkle treestructure). So the Merkle tree structure we use here is a [Merkle Mountain Range](https://docs.grin.mw/wiki/chain-state/merkle-mountain-range/) (MMR) structure, which requires the minimal number of SLOADs and SSTOREs per leaf insertion.
