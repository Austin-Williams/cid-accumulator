...
// CBOR major types
const MT_UINT = 0, MT_NEGINT = 1, MT_BYTES = 2, MT_STRING = 3, MT_ARRAY = 4, MT_OBJECT = 5, MT_TAG = 6, MT_SIMPLE = 7;
...
function encodeUint(val: number, mt: number): Uint8Array {
  ...
  if (val < 0x100000000) return new Uint8Array([mt << 5 | 26, (val>>>24)&0xff, (val>>>16)&0xff, (val>>>8)&0xff, val&0xff]);
...
function encode(val: any): Uint8Array {
  ...
  if (typeof val === "number") {
    if (Number.isInteger(val)) {
      ...
      view.setUint8(0, 0xfb); view.setFloat64(1, val);
...
function decode(buf: Uint8Array): any {
  ...
  function _decode(): any {
    ...
    const head = buf[off++];
    const mt = head >> 5, val = head & 0x1f;
    // Unsigned int
    let n = val;
    if (val >= 24) {
      if (val === 24) n = buf[off++];
      else if (val === 25) { n = (buf[off++] << 8) | buf[off++]; }
      else if (val === 26) { n = (buf[off++] << 24) | (buf[off++] << 16) | (buf[off++] << 8) | buf[off++]; }
      else throw new Error("Unsupported int size");
    }
    switch(mt) {
      case MT_UINT: return n;
      case MT_NEGINT: return -1 - n;
      case MT_BYTES: { const v = buf.slice(off, off+n); off += n; return v; }
      case MT_STRING: { const v = new TextDecoder().decode(buf.slice(off, off+n)); off += n; return v; }
      case MT_ARRAY: { const arr = []; for (let i=0; i<n; i++) arr.push(_decode()); return arr; }
      case MT_OBJECT: { const obj: any = {}; for (let i=0; i<n; i++) { const k = _decode(); obj[k] = _decode(); } return obj; }
      case MT_TAG: {
        if (n === 42) {
