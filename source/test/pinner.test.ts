	...
	beforeEach(async () => {
		...
		vi.doMock("ethers", () => {
			const Contract = class {
				constructor() {}
				static getAddress(address: string) { return address }
...
describe("highestContiguousLeafIndex", () => {
		...
		it("should return highest index for contiguous rows", () => {
			pinner.db = { prepare: vi.fn().mockReturnValue({ all: vi.fn().mockReturnValue([
				{ leaf_index: 0 }, { leaf_index: 1 }, { leaf_index: 2 }
			]) }) } as any;
			expect(pinner.highestContiguousLeafIndex()).toBe(2);
		});
		it("should return index before gap", () => {
			pinner.db = { prepare: vi.fn().mockReturnValue({ all: vi.fn().mockReturnValue([
				{ leaf_index: 0 }, { leaf_index: 2 }, { leaf_index: 3 }
			]) }) } as any;
