...
export function getIpfsNamespace(
  ...
  return {
		...
		shouldPin,
		shouldProvide,
    getAndResolveCID: (cid: CID<unknown, 113, 18, 1>, opts?: { signal?: AbortSignal }) => getAndResolveCID(ipfs, storage, cid, opts),
    rePinAllDataToIPFS: () => rePinAllDataToIPFS(ipfs, storage, shouldPut, shouldPin, shouldProvide),
    putPinProvideToIPFS: (cid: CID<unknown, 113, 18, 1>, dagCborEncodedData: DagCborEncodedData) => putPinProvideToIPFS(ipfs, shouldPut, shouldProvide, cid, dagCborEncodedData)
  }
}