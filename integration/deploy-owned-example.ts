...
async function main() {
	const providerUrl = process.env.ETHEREUM_HTTP_RPC_URL
	const mnemonic = process.env.MNEMONIC_FOR_SUBMITTER
	if (!providerUrl || !mnemonic) {
		throw new Error("Missing env: ETHEREUM_HTTP_RPC_URL or MNEMONIC_FOR_SUBMITTER")
	}
